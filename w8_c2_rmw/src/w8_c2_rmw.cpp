...
int atomic_fetch_add(atomic_int *x, int arg) {
  ...
  if (store_conditional(x, old_value + arg)) return old_value;
  else atomic_fetch_add(x, arg);
}
...
bool atomic_compare_exchange(atomic_int *x, int *expected_value,
  ...
  if (old_value == *expected_value) {
    if (store_conditional(x, new_value)) return true;
    else atomic_compare_exchange(x, expected_value, new_value);
  } else {
